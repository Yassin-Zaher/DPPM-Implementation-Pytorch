...
"""
EasyDict({
    "beta": tensor([1.9e-06, 2.0e-06, ..., 9.9e-01], 
                   device='cuda:0', dtype=torch.float32, shape=torch.Size([1000])),
                   
    "alpha": tensor([9.9e-01, 9.9e-01, ..., 1.0e-04],
                    device='cuda:0', dtype=torch.float32, shape=torch.Size([1000])),
                    
    "alpha_bar": tensor([9.9e-01, 9.9e-01, ..., 8.5e-06],
                        device='cuda:0', dtype=torch.float32, shape=torch.Size([1000])),
                        
    "beta_tilde": tensor([1.9e-06, 2.0e-06, ..., 1.2e-02],
                         device='cuda:0', dtype=torch.float32, shape=torch.Size([1000])),
                         
    "beta_log": tensor([-13.1, -13.0, ..., -0.0001],
                       device='cuda:0', dtype=torch.float32, shape=torch.Size([1000])),
                       
    "beta_tilde_log": tensor([-13.1, -13.0, ..., -4.4],
                            device='cuda:0', dtype=torch.float32, shape=torch.Size([1000]))
})
"""
class GuassianDiffusion:
    ...
    def __init__(self, timesteps=1000, device="cuda:0"):
        ...
        # Helper function to restrict values being between -1 and 1
        self.clamp_x0 = lambda x: x.clamp(-1, 1)
        # Given the predicted eps (noise added) return x0 (original image)
                ...
                np.array(
                    [
                        min(
                            ...
                            0.999, # This is a safety clamp to prevent βt​ from ever being 1.0, which would cause division-by-zero errors.
                        )
...
class loss_logger:
    ...
    def log(self, v, display=False):
        ...
        if self.ema_loss is None:
            ...
            # 90% of the previous smoothed loss and only 10% of the new raw loss
...
def train_one_epoch(
    ...
    for step, (images, labels) in enumerate(dataloader):
        ...
        # update ema_dict
        # model is often more stable and produces much better-looking images during sampling (inference).
...
def sample_N_images_specific_class(
    N,
    specific_class, # <-- New argument
    model,
    diffusion,
    xT=None,
    sampling_steps=250,
    ...
    """Samples N images of a SPECIFIC class.

    Args:
        N : Number of images
        specific_class : The class label (int) to generate.
        model : Diffusion model
    ...
    with tqdm(total=math.ceil(N / (args.batch_size * num_processes))) as pbar:
        while num_samples < N:
            ...
            # --- MODIFIED PART ---
            if args.class_cond:
                # Use the specific class for the entire batch
                y = torch.tensor([specific_class] * len(xT), dtype=torch.int64).to(
                    args.device
                )
            else:
                y = None
            ...
            if is_distributed:
                ...
                # Not distributed, just append results from this one process
                if args.class_cond:
                    labels.append(y.detach().cpu().numpy())
                samples.append(gen_images.detach().cpu().numpy())
                num_samples += len(xT)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If set, generate images of only this class label using the specific sampling function.",
    )
    ...
    if args.sampling_only:
        if args.specific_class is not None:
            print(f"--- Sampling {args.num_sampled_images} images of SPECIFIC CLASS: {args.specific_class} ---")
            sampled_images, labels = sample_N_images_specific_class(
                args.num_sampled_images,
                args.specific_class, # <-- Pass the class in
                model,
                diffusion,
                None,
                args.sampling_steps,
